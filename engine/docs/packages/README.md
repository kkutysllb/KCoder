# Qiongqi v1.1.1 逐包技术文档

本目录覆盖当前 18 个 workspace package。所有文档已按 v1.1.1 复核；引擎不包含产品 UI、固定 Agent、业务价值判断、secret 管理或默认模型。

## Foundation

| 包 | 文档 | v1.1 边界 |
|---|---|---|
| `@qiongqi/contracts` | [contracts.md](./contracts.md) | graph/root errors、strict Kernel dispatch、identity、stream、ROI schema |
| `@qiongqi/domain` | [domain.md](./domain.md) | Thread/Turn/Item/Event 纯领域实体与 reducer |
| `@qiongqi/ports` | [ports.md](./ports.md) | root atomic commit、work claim renewal、model/tool 与 runtime 端口 |
...
| 包 | 文档 | v1.1 边界 |
|---|---|---|
| `@qiongqi/loop` | [loop-orchestrator.md](./loop-orchestrator.md) | root coordinator、prepare-first dispatch、evented_v2、Kernel、governor、ROI |
...
| `@qiongqi/adapter-storage` | [adapter-storage.md](./adapter-storage.md) | root aggregate atomicity、cumulative budgets、PostgreSQL locking |
