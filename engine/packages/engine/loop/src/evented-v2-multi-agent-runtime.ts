...
import type {
  AgentDispatchPreparationInput,
  AgentExecutionInput,
  AgentExecutor
} from './kernel-agent-executor.js'
...
export type EventedV2MultiAgentRuntimeOptions = {
  ...
  approvalGovernor?: Pick<GraphGovernor, 'resolveApproval'>
}
...
export class EventedV2MultiAgentRuntime {
  ...
  async load(runId: string): Promise<MultiAgentRun | undefined> {
    ...
    return run ? MultiAgentRunSchema.parse(run) : undefined
  }
  ...
  async dispatchActiveAgent(input: {
    ...
    prompt: string
    requestedBudget: AgentExecutionInput['requestedBudget']
    ...
    return this.withRunLock(input.runId, () => this.withRunMutationLease(input.runId, async (fence) => {
      ...
      const node = requireGraphNode(this.graph, current.activeNodeId)
      if (node.kind !== 'agent' && node.kind !== 'judge') {
        throw new Error(`active graph node is not model-driven: ${node.id}`)
      }
      const agentId = executionAgentId(node)
      const agentRun = [...current.agentRuns].reverse().find((candidate) =>
        candidate.nodeId === node.id && candidate.agentId === agentId)
      if (!agentRun) throw new Error(`AgentRun not found for node: ${node.id}`)
      if (agentRun.executionRef) return current
      if (node.kind === 'judge' && (input.sharedEvidenceRefs?.length ?? 0) === 0) {
        return this.options.runs.update(input.runId, (run) => this.rejectJudgeWithoutEvidence(run, node.id, agentId), updateOptions(fence))
      }
      ...
      if (this.options.dispatchPreparer) {
        return this.options.dispatchPreparer.prepareAgentDispatch(prepared, updateOptions(fence))
      }
      const executor = this.options.agentExecutor
      if (!executor) throw new Error('evented_v2 agent executor is not configured')
      const execution = await executor.execute(prepared)
      return this.options.runs.update(input.runId, (run) => MultiAgentRunSchema.parse({
        ...run,
        agentRuns: run.agentRuns.map((candidate) => candidate.agentRunId === agentRun.agentRunId
          ? { ...candidate, status: 'running', executionRef: execution.executionRef, updatedAt: this.options.nowIso() }
          : candidate),
  ...
  async completeAgentTask(input: {
    ...
    if (input.executionRef?.parentKind !== undefined && input.executionRef.parentKind !== 'agent') {
      throw new Error('Kernel completion must reference an agent child run')
    }
    const completionAgentRunId = input.executionRef?.parentKind === 'agent'
      ? input.executionRef.agentRunId
      : undefined
    const completionKernelRunId = input.executionRef?.kernelRunId
    return this.withRunLock(input.runId, () =>
        ...
        const next = await this.options.runs.update(input.runId, (current) => {
          if (completionAgentRunId) {
            ...
            if (delivered.outcome) {
              if (delivered.executionRef?.kernelRunId !== completionKernelRunId
                || JSON.stringify(delivered.outcome) !== JSON.stringify(input.outcome)) {
                throw new Error(`Conflicting Kernel completion: ${completionKernelRunId ?? '<missing>'}`)
              }
              return current
            }
          }
          ...
          if (activeNode.kind !== 'agent' && activeNode.kind !== 'judge') {
            throw new Error(`Active node must be model-driven to complete task: ${activeNode.id}`)
          }
          const expectedAgentId = executionAgentId(activeNode)
          if (expectedAgentId !== input.agentId) {
          ...
          const agentRunIndex = input.executionRef
            ? current.agentRuns.findIndex((agentRun) => agentRun.agentRunId === completionAgentRunId)
            : latestAgentRunIndex(current, input.agentId, activeNode.id)
  ...
  async consumeKernelCompletion(input: { runId: string; completion: unknown }): Promise<MultiAgentRun> {
    const completion = KernelCompletionPayloadSchema.parse(input.completion)
    const executionRef = completion.executionRef
    if (executionRef.parentKind !== 'agent') {
      throw new Error('Kernel completion must reference an agent child run')
    }
    const run = await this.options.runs.load(input.runId)
    if (!run) throw new Error(`MultiAgentRun not found: ${input.runId}`)
    const agentRun = run.agentRuns.find((candidate) => candidate.agentRunId === executionRef.agentRunId)
    if (!agentRun) throw new Error(`AgentRun not found for Kernel completion: ${executionRef.agentRunId}`)
  ...
  async resolveApproval(input: ApprovalResolutionInput): Promise<MultiAgentRun> {
    const governor = this.options.approvalGovernor
    if (!governor) throw new Error('durable approval governor is not configured')
    const run = await governor.resolveApproval(input)
  ...
  private enterGraphNode(run: MultiAgentRun, nodeId: string): MultiAgentRun {
    ...
    if (node.kind === 'judge') {
      const agentId = executionAgentId(node)
      const hasJudgeRun = run.agentRuns.some((agentRun) =>
        agentRun.agentId === agentId && agentRun.nodeId === node.id && ['queued', 'running'].includes(agentRun.status))
      return MultiAgentRunSchema.parse({
        ...run,
        status: 'suspended',
        activeNodeId: node.id,
        agentRuns: hasJudgeRun ? run.agentRuns : [...run.agentRuns, {
          agentRunId: this.options.ids('agent_run'),
          agentId,
          nodeId: node.id,
          status: 'queued',
  ...
  private rejectJudgeWithoutEvidence(run: MultiAgentRun, nodeId: string, agentId: string): MultiAgentRun {
    const nextNodeId = nextNodeForCondition(this.graph, nodeId, 'rejected')
    if (!nextNodeId) throw new Error(`Judge node has no rejected edge: ${nodeId}`)
    const now = this.options.nowIso()
    const completed = MultiAgentRunSchema.parse({
      ...run,
      agentRuns: run.agentRuns.map((agentRun) => agentRun.nodeId === nodeId && agentRun.agentId === agentId
        ? { ...agentRun, status: 'completed', completedAt: now, updatedAt: now, summary: 'rejected: missing shared evidence' }
        : agentRun),
      events: [...run.events, {
        eventId: this.options.ids('mae'),
        type: 'node_completed',
        nodeId,
        agentId,
        payload: { condition: 'rejected', reason: 'missing_shared_evidence' },
        timestamp: now
      }],
      updatedAt: now
    })
...
function promptInputRef(run: MultiAgentRun): string {
  ...
  if (!event) throw new Error(`no durable prompt event is available for run: ${run.runId}`)
  return `graph-run://${run.runId}/events/${encodeURIComponent(event.eventId)}/prompt`
}
...
function executionAgentId(node: Extract<AgentGraphNode, { kind: 'agent' | 'judge' }>): string {
  return node.kind === 'agent' ? node.agentId : `judge:${node.id}`
}
...
function completionCondition(outcome: RunOutcome): string {
