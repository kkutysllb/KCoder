...
**Durable Engine v1.1.1 | 纯引擎代码 | 模型无关 | 可恢复治理图执行**
...
- **版本化治理图**：`compileAgentGraph()` 生成 canonical digest，`publishGraph()` 持久化 immutable revision；agent、tool、judge、join、wait、retry 和 terminate node 共用确定性 traversal。
...
- **真实实时流**：模型 delta、工具进度、work graph、checkpoint、usage、ROI 和 outcome 进入带 `streamId + seq` 的 durable stream；订阅者从 cursor replay 并独立 ack。
- **实时 ROI 与效率**：`recordCost()` / `recordValue()` 发布 `roi.snapshot`；可按 graph/node/edge 归因 cost/value，统计 fan-out、retry amplification、suppressed attempts、avoided cost 和执行 critical path。业务 ROI 与引擎效率分开呈现。
...
| v1.1.1 发布说明 | [docs/releases/v1.1.1.md](./docs/releases/v1.1.1.md) |
