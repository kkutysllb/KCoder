 ...
 * - 注入物：navList 尾部克隆一个按钮（标签改「技能」；图标换成上游
 *   IconSkillOutline16 path 零误差复制——克隆按钮自带上游齿轮，与通用
 *   设置同形），内容区追加
 ...
 *
 * 通信（console 通道，同 mcp-settings / about-settings 等注入器）：
...
const PAGE_JS = `(() => {
  ...
  // 导航图标工厂：上游 IconSkillOutline16 path 零误差复制（DOM 构造，
  // 不用 innerHTML——防 svg 序列化微任务风暴）；class 抄被替换的旧
  // svg（上游 navIcon 编译类，对齐随上游样式走）。
