 ...
 *   序列：侧栏面板 12 / 内嵌终端 44 / 上下文 76 / git 108（theme-watcher
 *   避让带同步 134）；徽章显示相对 HEAD 的 +N −N 行数（numstat 求和，
 *   agent 连续编辑也反映）；
 * - 面板：透明背景 WebContentsView，bounds 为右侧浮动矩形（不盖
 *   标题栏）；show 时重挂到 contentView 末尾置顶（防终端面板后开时
 *   z-order 压住卡片）；panel-menu 下拉打开时 yieldForMenu 临时
 *   收起（视图层盖住页面 DOM，z-index 无解）；
...
class GitPanel {
   ...
   * 只能临时收起视图；visible 态/pad/轮询全部保留，关闭即原样恢复。
   */
  yieldForMenu(on: boolean): void {
    if (this.view === null) return
    if (on) this.view.setVisible(false)
  ...
  /**
   * 用系统默认应用打开计划文档（git 面板计划区点击；预览抽屉
   * 已移除，markdown 由系统关联应用渲染）。
   */
