 ...
 *    左侧注入 26x26 折叠按钮——macOS 红绿灯右侧（left 78px）/
 *    Windows 最左（left 12px）：
 ...
 *    = 34（按钮宽 26 + 间距 8），theme-watcher 的 margin-left /
...
const PAGE_JS = `(() => {
  ...
  const EXTRA_LEFT = 34 // 按钮宽 26 + 间距 8
...
export function attachSidebarToggle(win: BrowserWindow): void {
  ...
  const left = process.platform === 'win32' ? 12 : 78
  const script = PAGE_JS.replaceAll(PLACEHOLDER, String(left))
