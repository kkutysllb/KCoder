...
| `0006-opencode-session-header.patch` | opencode 网关（Go/Zen）自某时点起对缺 `x-opencode-session` 的请求直接回 400 MissingSessionID（会话亲和路由/prompt cache 需要稳定会话 id）；pi-ai 库从不发该头（pi 上游的修复在 coding-agent/sdk.ts 层，pi-ai 0.84.2→0.85.1 均无此逻辑），llm-pi-ai 直连必挂。adapter 在构造 `streamOptions.headers` 时为 opencode/opencode-go/ln 端点（provider 名或 baseUrl 含 opencode.ai）注入 `x-opencode-session`：值取 seam 盖印的会话 id（`options.sessionId`，conversation 级稳定），无则用 per-adapter 稳定 uuid 兜底；部署显式配置的头（任意大小写）优先。含 6 个单元用例 + 4 个 egress 集成测试（mock server 实收请求头断言）；llm-pi-ai 全包 298/298 绿 + oxlint 0/0 | fork 分支 `fix/opencode-session-header`（提交 facdc190a2 已在本地集成分支 kcoder/0.1.3-alpha.2 上，随 51ac894c merge 带入 alpha.2；分支推送 fork 后可 format-patch 覆盖归档——当前归档即该提交的 diff） |
| `0007-user-message-edit.patch` | 用户气泡「编辑并重发」回归：编辑入口随附件插件线退役（v0.5.1 宿主补丁链）而消失，上游从未发布同会话消息编辑（#3452 当天撤，净零）。引擎侧纯 UI 恢复：ui-conversation 增 `ctx.uiConversation.fillDraft(sessionId, text)`（显式会话 id，绕开 scope-addressed 服务注入限制——直接属性访问 `ctx.conversation` 会因未 inject 抛错；经 `SessionInput.setDraft` 整稿替换 + caret 落位）；ui-chat 的 ChatViewInjected/ChatNodeOwnerProps 增可选 `editUserMessage`（apply 接线到 uiConversation.fillDraft），`MessageIconActions` 增可选 onEdit（铅笔，copy 前），user/steering 气泡传入该消息文本。不动会话存储与 agent 循环；文本-only 回填。message-icon-actions 组件 spec（3 例）+ 全量 ui-chat/ui-conversation 回归绿 + oxlint 0/0。上游若未来提供真同会话编辑可整体撤销 | 提交 b13e950d94 已直接并入本地集成分支 kcoder/0.1.3-alpha.2（与 0006 同形态，无独立 fork 分支）；推送 fork 后随 v0.5.7 一起发布 |
