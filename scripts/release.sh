...
cmd_build() {
  ...
  #    基线断言（fork 锚定）：消费态 = 集成分支 kcoder/0.1.5-rc.1，历史必须含
  #    钉版基线（upstream/BASELINE）——手动改集成分支忘同步基线文件时在
  #    此拦下，未验证代码不进产物。
  BASELINE_SHA="$(grep -vE '^[[:space:]]*(#|$)' "$ROOT/upstream/BASELINE" | head -1 | tr -d '[:space:]')"
  [[ "$(git -C "$UPSTREAM" branch --show-current)" == "kcoder/0.1.5-rc.1" ]] \
    || die "上游克隆不在集成分支 kcoder/0.1.5-rc.1 上（先 bash scripts/setup.sh，或更新 upstream/BASELINE）"
