              ...
              <div className="space-y-1.5">
                {q.options.map((opt) => (
                  <button
                    ...
                    className={`w-full flex items-start gap-2.5 px-3 py-2 rounded-lg text-left text-xs transition-colors border ${
                      answers[q.id] === opt.label
                        ? 'bg-[#3b82f6]/10 border-[#3b82f6]/40 text-text-primary'
                  ...
                  >
                    <span className={`mt-0.5 w-3.5 h-3.5 shrink-0 rounded-full border-2 flex items-center justify-center ${
                      answers[q.id] === opt.label ? 'border-[#3b82f6]' : 'border-[#52525b]'
                    }`}>
                      {answers[q.id] === opt.label && <span className="w-1.5 h-1.5 rounded-full bg-[#3b82f6]" />}
                    </span>
                ...
                {q.options.length === 0 && (
                  <input
                    ...
                    disabled={submitting}
                    className="w-full bg-bg-input text-text-primary placeholder-text-muted rounded-lg px-3 py-2 text-xs outline-none border border-border-custom focus:border-[#3b82f6]"
                  />
                )}
              </div>
            </div>
