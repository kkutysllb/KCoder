...
AppConfig = qilin_app_config.AppConfig
get_app_config = qilin_app_config.get_app_config
...
async def _ensure_admin_user(app: FastAPI) -> None:
    ...
    "no-auth → with-auth" upgrade path: users who ran QiLin without
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # plain `import qilin.agents` never installs a process-global tracer.
    ...
    async with langgraph_runtime(app, startup_config):
        ...
        try:
            from qilin.community.browser_automation import get_browser_session_manager
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="QiLin API Gateway",
        description="""
## QiLin API Gateway

API Gateway for QiLin - A LangGraph-based AI agent backend with sandbox execution capabilities.
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
        ...
        openapi_tags=[
            ...
            {
                "name": "threads",
                "description": "Manage QiLin thread-local filesystem data",
            },
            {
                "name": "agents",
    ...
    # (or when the explicit QILIN_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS=1
    ...
    async def health_check() -> dict[str, str]:
        ...
        """
        return {"status": "healthy", "service": "qilin-gateway"}
