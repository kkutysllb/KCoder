...
### 9.10 产品铁律落档（2026-09-19：原生侧栏永不复用）

**决策（产品负责人原话）**：「后续不再使用上游原生的侧边栏功能，只针对
上游变化完善迭代我们侧边栏插件功能」；配套既定原则「新版本的适配只修改
插件源码发布新版本解决，不要在 KCoder 中适配」。

落点（文档 + 代码执行点，均随 v0.6.14 入库）：

| 位置 | 内容 |
|---|---|
| `docs/ARCHITECTURE.md` **§12 产品铁律**（新增节） | 铁律 1：不使用上游原生侧边栏功能——服务层保留 / 外壳收掉 / 打开动作归插件 / 按钮与入口自持 / 上游改动只走插件发版；铁律 2：新版本适配只改插件源码（KCoder 仅消费接线 + 宿主压制 + 自有功能；宿主契约例外） |
| 同 §7 引言、§10 速查表 | 交叉引用：§7 说「改哪里」，§12 说「哪些不做」；新增一行「⚠ 上游动了侧边栏 → 只改插件仓发新版本，绝不接回原生」 |
| 同 §8 协作惯例 | 发版链（`release.sh ship`）的 commit/tag/push 为流程既定步骤，是「push 由用户执行」的唯一例外；并记 fork 集成分支必须先于 tag 推送（否则 CI 发布物与本地验证脱节） |
| `desktop/main/style-overlay.ts` | `NATIVE_SIDEBAR_CSS` 注释标注为铁律 1 的执行点：上游改外壳名或新增侧栏形态时，正解是改插件发版，不是放开压制 |
| `desktop/main/sidebar-cluster.ts` | 同上：开关永远转发插件自己的按钮，不为上游原生侧栏开关契约加分支 |

**与 §9.9 的关系**：§9.9 是「撤销退役决策」（un-retire，可再评估）；本节把
「右侧工作台由自研插件承担」升级为**不可回退的铁律**（要改须显式拍板并留痕）。

**已知副作用（须在发布说明列明）**：上游侧栏新形态——侧栏浏览器
`ui-sidebar-browser`（消息内 HTTP(S) 链接默认进侧栏 browser tab，走
`ctx.sidebarRight.openTab` 内部路由，**不经我们的 openpath 拦截面**）——按
