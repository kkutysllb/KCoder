...
> **已自动化（两层）**：插件仓 `pnpm check:contract`
> ① 枚举每个 `openTab` 调用点——内容型打 ✓、有就地标注的 type-only 打 ○、既非内容型又无标注即失败
> （逼作者对每个调用点做一次显式决定）；
> ② **钉住判据本身**：`src/client/service.ts` 里 `openTab` 的「内容型」条件必须同时认
> `path` / `url` / `meta`，缺任一即失败——抓的正是下面那个现场（`meta` 不在判据里）。
>
> ⚠️ **真行为测试尚未落地**（诚实标注）：行为级验证要把判据抽成无依赖纯模块（那样
> `unrun` 才能直接对真源码跑用例），但抽取会改 `lib/**`（运行时面）⇒ 按 §2 的版本线规则
> 就得 bump 版本 + 发布 + 平移声明 + 再发一次桌面版。为一个「行为不变的可测性重构」走整条
> 发版链不成比例，故**并进下一次本来就要 bump 的版本**（那批功能一起做）。在那之前，这条
> 规则由「形状断言 + 调用点枚举」守着，仍好过只靠实机点。
