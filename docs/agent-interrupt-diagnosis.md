...
| 8 | 工具执行挂起 | ✅ bash_command_timeout=600s 工具超时 + 840s 看门狗 |
| 9 | LLM 连接挂起 | ✅ 模型客户端超时 + 看门狗 |
| 10 | 子代理卡死 | ✅ **子代理超时 1800s → 300s（2026-08-16）**：卡死子代理 5 分钟自动超时，lead 收到 task_failed 继续执行，不再沉默 30 分钟。task_tool 轮询只在子代理有新消息时发事件，卡死=零事件，840s 看门狗兜底 |
| 11 | 内存压力系统杀进程 | ✅ 缓解（checkpoint delta） |
| 12 | **遗留 run 幽灵恢复** | ⚠️ langgraph_runtime_inmem 持久化 pending run，app 重启自动恢复执行但前端/网关无订阅 → 后台静默执行。修复方向：app 退出时 cancel 遗留 run / 前端恢复感知（待实施） |
| 13 | Electron SIGSEGV 崩溃 | ⚠️ 2026-08-16 上午实测 5 次（11:48-11:56），app 突然消失任务中断。需排查（GPU/内存/渲染进程） |
