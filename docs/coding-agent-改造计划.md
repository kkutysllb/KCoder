...
| D | 技能与场景完善 | ✅ 完成（D1 技能深接 / D2 错误自愈 / D3 交付增强），⏳ 待运行时验收 |
...
## 6. 阶段 D：技能与场景完善（✅ 已实现，⏳ 待运行时验收）

**确认结论（用户 2026-08-15）**：D1+D2+D3 全部纳入；推送永不自动化（默认关闭）。

- D1 **技能在 coding 闭环中的深接** ✅：
  - 「开工即检查」提示词强化：复杂任务开始前查匹配技能（describe_skill / 读 SKILL.md），结合 repo_map/dep_map 的仓库上下文；
  - **自定义技能安装后即时可用**：引擎技能缓存增加目录 mtime 指纹——gateway 进程安装（跨进程）后缓存命中时检测到指纹变化即重扫，不重启引擎即可用；
  - 技能产物与工具的配合随 skill-first 提醒落地（技能是说明不是交付物）。
- D2 **错误自愈** ✅：
  - 新增 `ToolRetryMiddleware`：只读工具（ls/read_file/glob/grep/repo_map/dep_map/security_scan/web_search 等）「Error:」瞬态失败同参自动重试一次；mutating 工具绝不重试（防重复副作用）；审批 BLOCKED 不误重试；
  - <verification> 强化：失败必须读错误输出、定位根因再改（红绿循环强制）。
- D3 **交付增强** ✅：
  - changelog 追加**去重**（writeChangelog 指令：先读现有内容，相同/相似条目跳过）；
  - PR 描述模板化：标题/概述/变更/验证清单（checklist）/审查结论；
  - **分支 + 提交**：git 仓库内非平凡任务开工创建 `kcoder/<slug>` 分支、交付时 commit；**NEVER push**（推送默认关闭，永不自动化）。
