...
- 技能在 coding 闭环中的深接：skill-first 触发、自定义技能安装后的即时可用、技能产物（SKILL.md/脚本）与 repo_map/dep_map 的配合。
- 更多交付场景：分支创建 + 推送（可选）、PR 模板、changelog 去重。
- 错误自愈：工具失败后的自动重试/降级策略。

> 具体条目在 C 完成后细化，先占位防止遗忘。
