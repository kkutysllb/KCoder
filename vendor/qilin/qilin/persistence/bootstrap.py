"""Hybrid schema bootstrap for QiLin's application tables.
...
| DB state                              | Action                                  |
|---------------------------------------|-----------------------------------------|
| empty (no QiLin tables)            | ``create_all`` + ``alembic stamp head`` |
| legacy (QiLin tables, no alembic)  | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
QiLin-owned table. ``create_all`` runs first because stamping at
...
def _reflect_state(sync_conn: Any) -> dict[str, bool]:
    ...
    - ``has_qilin_tables``: True iff at least one table that ``Base.metadata``
    ...
    try:
        ...
        logger.debug("qilin.persistence.models not found; metadata may be incomplete")
    ...
    return {
        ...
        "has_qilin_tables": bool(reflected & metadata_tables),
    }
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all QiLin-owned tables on *sync_conn*."""
    ...
    try:
        ...
        logger.debug("qilin.persistence.models not found; bootstrap will create empty schema")
...
def _run_baseline_create_all_sync(sync_conn: Any) -> None:
    ...
    try:
        ...
        logger.debug("qilin.persistence.models not found; baseline backfill may be incomplete")
...
async def _sqlite_lock(engine: AsyncEngine):
    ...
    QiLin. The 30s ``busy_timeout`` plus idempotent revisions cover the
