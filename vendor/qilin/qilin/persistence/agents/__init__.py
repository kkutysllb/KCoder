...
:mod:`qilin.config.agents_config` dispatch to. ``file`` (default) preserves
...
def make_agent_store(config: AppConfig) -> AgentStore:
    ...
    if config.agent_storage.backend == "db":
        ...
        from qilin.persistence.agents.sql import SqlAgentStore
...
def _file_store() -> AgentStore:
    ...
    if _file_store_singleton is None:
        from qilin.persistence.agents.file import FileAgentStore
