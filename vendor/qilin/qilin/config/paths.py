...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable QiLin state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.qilin``).  Using
...
class Paths:
    """
    Centralized path configuration for QiLin application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. QILIN_HOME environment variable
        3. Caller project fallback: `{project_root}/.qilin`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set QILIN_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("QILIN_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("QILIN_HOME"):
