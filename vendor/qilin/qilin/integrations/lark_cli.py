...
* a **content** SHA-256 over the extracted skill tree, after QiLin's shared
...
also manages a writable QiLin-owned Gateway CLI under
``.qilin/integrations/lark-cli/gateway-cli`` and prefers it over the system
...
LARK_CLI_MANIFEST_FILE = ".qilin-lark-cli-manifest.json"
...
LARK_CLI_RUNTIME_MANIFEST_FILE = ".qilin-lark-cli-runtime.json"
...
def lark_cli_managed_gateway_dir() -> Path:
    """Gateway-scoped QiLin-managed lark-cli install root."""
...
def _ensure_managed_gateway_lark_cli() -> LarkCliProbe:
    """Install/update the QiLin-managed Gateway lark-cli.
...
def _content_sha256(root: Path, skill_names: set[str]) -> str:
    ...
    The caller computes this after injecting QiLin's shared guidance, so the
...
def _append_qilin_lark_shared_guidance(root: Path) -> None:
    ...
    if _QILIN_LARK_SHARED_GUIDANCE_MARKER in content:
        return
    for legacy_marker in _QILIN_LARK_SHARED_GUIDANCE_LEGACY_MARKERS:
        if legacy_marker in content:
            content = content.split(legacy_marker, maxsplit=1)[0].rstrip()
            break
    guidance = f"""

{_QILIN_LARK_SHARED_GUIDANCE_MARKER}

## QiLin 授权入口

在 QiLin 中，如果 `lark-cli auth status` 或业务命令提示未配置、未登录、token 过期或缺少用户授权：
