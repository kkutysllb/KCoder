...
# shared ``qilin.config.file_signature`` helper also used by
# ``qilin.config.app_config`` for the sibling runtime-editable config file —
...
def _resolve_config_path() -> Path | None:
    ...
    when an explicit `config_path` or `QILIN_EXTENSIONS_CONFIG_PATH`
...
async def initialize_mcp_tools() -> list[BaseTool]:
    ...
    async with _initialization_lock:
        ...
        from qilin.mcp.tools import get_mcp_tools
