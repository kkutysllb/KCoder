...
from qilin.community.warm_pool_lifecycle import (
...
DEFAULT_PORT = 8080
DEFAULT_CONTAINER_PREFIX = "qilin-sandbox"
...
class AioSandboxProvider(WarmPoolLifecycleMixin[SandboxInfo], SandboxProvider):
    ...
    Configuration options in config.yaml under sandbox:
        ...
        container_prefix: qilin-sandbox
    ...
    def _get_skills_mounts(*, user_id: str | None = None) -> list[tuple[str, str, bool]]:
        ...
        Mount sources use ``QILIN_HOST_SKILLS_PATH`` and
        ``QILIN_HOST_BASE_DIR`` when running inside Docker (DooD) so the
        ...
        try:
            ...
            host_skills_root = os.environ.get("QILIN_HOST_SKILLS_PATH") or str(skills_path)
    ...
    def _lark_broker_active(user_id: str | None = None) -> bool:
        ...
        try:
            if not AioSandboxProvider._lark_integration_active(user_id):
                return False
            from qilin.integrations.lark_cli import sandbox_lark_broker_active
