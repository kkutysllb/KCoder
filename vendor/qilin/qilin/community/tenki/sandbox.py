"""``TenkiSandbox`` — QiLin :class:`Sandbox` backed by a Tenki cloud sandbox.
...
with the shared ``qilin.sandbox.search`` helpers, the same approach as
...
# ``/mnt`` is root-owned, so QiLin's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """QiLin Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: QiLin-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map QiLin virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        QiLin passes a bash command *string*; it runs through ``sh -lc``.
