"""``TenkiSandboxProvider`` — QiLin :class:`SandboxProvider` backed by Tenki.

Integrates `Tenki <https://tenki.cloud>`_ cloud sandboxes as a QiLin sandbox
...
_SANDBOX_NAME_PREFIX = "qilin-tenki-"
# Tenki terminates a sandbox at its max lifetime (~30 min by default), which
# would silently drop a long-running thread's state mid-conversation. QiLin
...
def _bootstrap_script(home_dir: str) -> str:
    """Materialise QiLin's virtual path layout inside the sandbox.
...
class TenkiSandboxProvider(WarmPoolLifecycleMixin[TenkiSandbox], SandboxProvider):
    """Run each QiLin sandbox as a Tenki cloud microVM."""
    ...
    def _create_sandbox(self, sandbox_id: str) -> TenkiSandbox:
        ...
        # Materialise QiLin's virtual path layout under the writable HOME.
