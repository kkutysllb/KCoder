...
from qilin.agents.middlewares.skill_context import (
    SKILL_CONTEXT_ENTRY_KEY,
    _tool_call_path,
    build_skill_entry_metadata_from_read,
)
from qilin.agents.middlewares.tool_result_meta import (
...
def _build_runtime_middlewares(
    *,
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "qilin"
    ...
    from qilin.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
    ...
    # chain includes ToolErrorHandlingMiddleware (inner), which stamps qilin_tool_meta
...
def build_subagent_runtime_middlewares(
    *,
    ...
    # QiLinSummarizationMiddleware — subagents inherit none of the lead's
