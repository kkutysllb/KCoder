...
and set ``manager_class: <name>``. Nothing else in qilin changes.
...
class MemoryManager(BaseModel):
    ...
    with backend configs (e.g. ``QiLinMemConfig``). Subclasses still MUST
    ...
    with the qilin conversation thread. The contract is deliberately
    neutral so a third-party memory system can be adapted without qilin
    code changes:

    - :meth:`get_context` returns plain injection text; the *format* is the
      implementation's own choice and is NOT part of the contract (QiLinMem
    ...
    # parse it (QiLinMem -> QiLinMemConfig) do so in model_post_init / from_config.
    ...
    def _check_invariants(self) -> MemoryManager:
        ...
        (bypassing the factory), since it lives on the base model. QiLinMem-private
        invariants (e.g. storage_path is a directory) stay on ``QiLinMemConfig``.
    ...
    def warm(self) -> bool | None:
        ...
        Return contract (tri-state so the host logs accurately):
          ...
          * ``None``  -- this backend has nothing to warm (the default). The
            ...
            successfully", so a non-QiLinMem backend doesn't claim a tiktoken
...
def _resolve_manager_class(manager_class: str) -> type[MemoryManager]:
    ...
    state, so silently substituting QiLinMem when an explicit ``manager_class``
...
# LLM). The portable backend package never names a qilin concept; the host
# supplies them HERE (host code outside ``backends/qilinmem/``). The factory
# passes them to ``cls.from_config(..., **host_hooks)``; each backend's
# ``from_config`` consumes the ones it needs (QiLinMem does; noop ignores them).
    ...
    def on_memory_llm_call(
        ...
        *,
        ...
        inject_langfuse_metadata(
            ...
            environment=os.environ.get("QILIN_ENV") or os.environ.get("ENVIRONMENT"),
            qilin_trace_id=trace_id,
        )


def _host_default_should_keep_hidden_message(additional_kwargs: Any) -> bool:
    """qilin default for QiLinMem's ``should_keep_hidden_message`` slot.
...
def _host_default_llm() -> Any:
    """qilin default for QiLinMem's ``host_llm`` slot (zero-config extraction).
    ...
    configured) so QiLinMem no-ops extraction with a clear error rather than
    crashing startup.
    """
    try:
...
def _host_default_extraction_callback(payload: Any) -> None:
    """qilin default for QiLinMem's ``extraction_callback`` slot.
    ...
    are never raised (the QiLinMem side already wraps the call).
    """
...
def get_memory_manager() -> MemoryManager:
    ...
    # qilin is multi-threaded: memory injection runs via asyncio.to_thread,
    ...
    # dependencies (QiLinMem owns its storage/queue/updater; others may open
    ...
    with _manager_lock:
        ...
        # Zero-config UX: default QiLinMem storage to qilin's state dir
        # (absolute, CWD-independent) so memory lands at
        # {runtime_home}/users/{user_id}/memory.json (qilin's base_dir,
        # same as pre-abstraction) unless the host explicitly sets storage_path.
        if not backend_config.get("storage_path"):
            from qilin.config.runtime_paths import runtime_home
        ...
        # storage_path-is-a-file guard lives on QiLinMemConfig.model_validator
        # now (QiLinMem-private semantics; fires even when the factory bypassed).
