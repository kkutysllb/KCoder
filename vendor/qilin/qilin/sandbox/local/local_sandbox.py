...
from qilin.sandbox.env_policy import build_sandbox_env
from qilin.sandbox.local.list_dir import list_dir
...
from qilin.sandbox.search import GrepMatch, find_glob_matches, find_grep_matches
...
class LocalSandbox(Sandbox):
    ...
    def _reverse_output_patterns(self) -> list[re.Pattern[str]]:
        ...
        # ``qilin.sandbox.path_patterns`` and shared with
    ...
    def _run_posix_command(
        ...
        stdout_capture, stdout_thread = LocalSandbox._start_pipe_drain(stdout_read_fd, "qilin-bash-stdout-drain")
        stderr_capture, stderr_thread = LocalSandbox._start_pipe_drain(stderr_read_fd, "qilin-bash-stderr-drain")
        try:
