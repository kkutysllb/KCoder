...
from qilin.skills.review.eval_schema import analyze_eval_manifests
...
from qilin.skills.review.resource_graph import build_resource_graph
from qilin.skills.skillscan.orchestrator import scan_skill_dir


def analyze_skill_package(snapshot: dict[str, Any], *, profile: ProfileName = "qilin") -> dict[str, Any]:
    ...
    if unexpected:
        findings.append(
            make_finding(
                ...
                remediation="Remove unsupported fields or add them to the shared QiLin frontmatter schema.",
                evidence=unexpected,
            )
        )
    ...
    if not isinstance(description, str) or not description.strip():
        ...
        findings.append(
            make_finding(
                ...
                severity="error",
                path="SKILL.md",
                message="Description exceeds QiLin's 1024 character limit.",
