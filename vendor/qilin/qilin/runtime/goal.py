...
Gateway runs and thin API surfaces. It intentionally lives in ``qilin`` so
...
def create_goal_evaluator_model(
    *,
    ...
    ``QiLinClient.stream``, which attach ``build_tracing_callbacks()`` at
...
def _resolve_environment() -> str | None:
    return os.environ.get("QILIN_ENV") or os.environ.get("ENVIRONMENT")
...
async def evaluate_goal_completion(
    ...
    *,
    ...
    ``thread_id``/``user_id``/``qilin_trace_id`` are forwarded to Langfuse
