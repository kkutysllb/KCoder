...
run path resolve it through :func:`qilin.config.app_config.get_app_config`,
...
if TYPE_CHECKING:
    ...
    from qilin.persistence.thread_meta.base import ThreadMetaStore
    from qilin.runtime import RunRecord
...
def get_config() -> AppConfig:
    ...
    Routes through :func:`qilin.config.app_config.get_app_config`, which
    ...
    :func:`get_app_config` closes the bytedance/qilin issue #3107 BUG-001
    ...
    :mod:`qilin.config.reload_boundary` and is mirrored by the
...
async def langgraph_runtime(app: FastAPI, startup_config: AppConfig) -> AsyncGenerator[None, None]:
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    async with AsyncExitStack() as stack:
        ...
        if sf is not None:
            from qilin.persistence.feedback import FeedbackRepository
            from qilin.persistence.run import RunRepository
        ...
        if sf is not None:
            ...
            from qilin.persistence.scheduled_tasks import ScheduledTaskRepository
