"""ChannelManager — consumes inbound messages and dispatches them to the QiLin agent via Gateway."""
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from QiLin Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the QiLin operator."
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical QiLin user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound QiLin owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the QiLin agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives QiLin-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the QiLin account that
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # enforced at the webhook route by HMAC, and the "sender → QiLin
        ...
        # the binding by provider identity before creating QiLin threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing QiLin thread, creating one if this is the
