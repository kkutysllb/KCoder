...
class DiscordChannel(Channel):
    ...
    def __init__(self, bus: MessageBus, config: dict[str, Any]) -> None:
        ...
        # conversations to QiLin thread IDs — a different concern.
        ...
        if store is not None:
            ...
            self._thread_store_path = Path.home() / ".qilin" / "channels" / "discord_threads.json"
    ...
    async def _bind_connection_from_connect_code(self, message, code: str) -> bool:
        ...
        await self._send_connection_reply(message, "Discord connected to QiLin.")
        return True
    ...
    async def _create_thread(self, message):
        try:
            ...
            thread_name = f"qilin-{message.author.display_name}-{message.id}"[:100]
