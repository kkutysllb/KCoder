...
This lets the existing :class:`~qilin.guardrails.middleware.GuardrailMiddleware`
enforce :class:`~qilin.authz.provider.AuthorizationProvider` decisions at
tool-call time — no new middleware class required (see RFC §6.1).

The adapter maps :class:`~qilin.guardrails.provider.GuardrailRequest`
fields to :class:`~qilin.authz.provider.AuthzRequest` fields, calls the
authorization provider, and converts the :class:`~qilin.authz.provider.AuthzDecision`
back to a :class:`~qilin.guardrails.provider.GuardrailDecision`.

Principal construction delegates to
:func:`~qilin.authz.principal.build_principal_from_context` so Layer 1
...
class GuardrailAuthorizationAdapter:
    ...
    def evaluate(self, request: GuardrailRequest) -> GuardrailDecision:
        ...
        adapter is consumed by :class:`~qilin.guardrails.middleware.GuardrailMiddleware`,
