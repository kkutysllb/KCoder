"""Pure-argument factory for QiLin agents.

``create_qilin_agent`` accepts plain Python arguments — no YAML files, no
...
def create_qilin_agent(
    ...
    *,
    ...
    """Create a QiLin agent from plain Python arguments.
    ...
    if checkpoint_channel_mode == "delta" and checkpointer is not None:
        raise ValueError(
            "create_qilin_agent does not support checkpoint_channel_mode='delta' with a checkpointer: "
            "persisted graphs built here bypass checkpoint mode marker injection and the fail-closed "
            "compatibility gate (see qilin.runtime.checkpoint_mode), so a mixed-mode store would "
            "silently corrupt thread state.  Use the guarded application paths (make_lead_agent or "
            "QiLinClient) for delta persistence; delta without a checkpointer is ephemeral and allowed."
        )
...
def _assemble_from_features(
    ...
    *,
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from qilin.tools.builtins import view_image_tool
