...
from qilin.tracing import build_tracing_callbacks
...
def _create_summarization_middleware(*, app_config: AppConfig | None = None, run_model_name: str | None = None) -> QiLinSummarizationMiddleware | None:
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching QiLin's style
    system_prompt = """
<todo_list_system>
...
def build_middlewares(
    ...
    *,
    ...
    ``make_lead_agent`` and by the embedded ``QiLinClient`` (a lead-agent variant
...
def _load_enabled_available_skills(available_skills: set[str] | None, *, app_config: AppConfig, user_id: str | None = None) -> list[Skill]:
    try:
        from qilin.agents.lead_agent.prompt import get_enabled_skills_for_config
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    from qilin.runtime.user_context import resolve_config_user_id
