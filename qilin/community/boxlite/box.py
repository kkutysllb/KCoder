"""``BoxliteBox`` — QiLin :class:`Sandbox` backed by a BoxLite micro-VM.

QiLin's ``Sandbox`` contract is synchronous; BoxLite's SDK is async-native and
...
``asyncio.to_thread`` worker QiLin invokes us from.

Every operation is a shell command run inside the box (``cat`` / ``find`` /
``grep`` / chunked ``base64``), parsed with the shared ``qilin.sandbox.search``
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: QiLin-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so QiLin's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        QiLin passes a bash command *string*; BoxLite's ``exec`` takes argv, so
