...
from qilin.config.run_ownership_config import RunOwnershipConfig
from qilin.config.runtime_paths import existing_project_file
from qilin.config.safety_finish_reason_config import SafetyFinishReasonConfig
from qilin.config.sandbox_config import SandboxConfig
from qilin.config.scheduler_config import SchedulerConfig
...
def is_trace_correlation_enabled(config: Any) -> bool:
    ...
    the Gateway ``TraceMiddleware`` and the embedded ``QiLinClient`` so
    the two entry points cannot drift on when ``qilin_trace_id`` is
...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``qilin``/``app`` logger hierarchies.

    Only the ``qilin`` and ``app`` logger levels are changed so that
...
class AppConfig(BaseModel):
    """Config for the QiLin application"""

    log_level: str = Field(
        default="info",
        description=format_field_description(
            "log_level",
            field_doc="Logging level for qilin and app modules (debug/info/warning/error); third-party libraries are not affected.",
        ),
    )
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `QILIN_CONFIG_PATH` environment variable, use it.
    ...
    def _apply_singleton_configs(cls, config: Self, acp_agents: dict[str, ACPAgentConfig]) -> None:
        from qilin.config.checkpointer_config import get_checkpointer_config
        ...
        if previous_checkpointer_config != config.checkpointer:
            ...
            from qilin.runtime.checkpointer import reset_checkpointer
            from qilin.runtime.store import reset_store
