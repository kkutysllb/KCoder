...
Lives at the top-level package (not ``qilin.runtime``) so it can be
imported from ``qilin.agents.thread_state`` without pulling in the heavy
``qilin.runtime`` package __init__ (which eagerly imports the runs
machinery). Anchored from ``qilin.agents.thread_state`` so every process
that builds a QiLin graph (gateway, workers, in-process LangGraph
...
_BINOP_PATCH_FLAG = "_qilin_overwrite_first_write_patched"
...
def _as_overwrite(value: Any) -> tuple[bool, Any]:
    ...
    QiLin's write paths produce for these Union channels (the branch and
    ...
    an internal serialization detail QiLin never emits into these channels.
    """
