...
from qilin.agents.thread_state import SandboxState, ThreadDataState, ThreadState
...
from qilin.subagents.config import SubagentConfig, resolve_subagent_model_name
from qilin.subagents.step_events import capture_new_step_messages
from qilin.subagents.token_collector import SubagentTokenCollector
from qilin.trace_context import QILIN_TRACE_METADATA_KEY
...
if TYPE_CHECKING:
    ...
    # `from qilin.subagents import SubagentExecutor`, which re-enters this
    # still-initializing package. Type-only here keeps the annotation precise.
    from qilin.tools.builtins.tool_search import DeferredToolSetup
...
def _extract_llm_error_fallback(final_state: Any) -> str | None:
    ...
    for message in reversed(final_state.get("messages", [])):
        ...
        if metadata.get("qilin_error_fallback") is not True:
            return None
...
def _copy_isolated_subagent_context() -> Context:
    ...
    if isinstance(callbacks, BaseCallbackManager):
        ...
        isolated_callbacks.inheritable_handlers = [handler for handler in callbacks.inheritable_handlers if not getattr(handler, "qilin_loop_bound", False)]
    elif isinstance(callbacks, (list, tuple)):
        isolated_callbacks = [handler for handler in callbacks if not getattr(handler, "qilin_loop_bound", False)]
...
class SubagentExecutor:
    ...
    def _create_agent(self, tools: list[BaseTool] | None = None, *, deferred_setup: "DeferredToolSetup | None" = None):
        ...
        if deferred_setup is not None and deferred_setup.deferred_names:
            from qilin.tools.builtins.tool_search import build_mcp_routing_middleware
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        if skills:
            if skill_setup.skill_names:
                ...
                from qilin.agents.lead_agent.prompt import get_skills_prompt_section
    ...
    async def _aexecute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            inject_langfuse_metadata(
                ...
                qilin_trace_id=self.qilin_trace_id,
            )
