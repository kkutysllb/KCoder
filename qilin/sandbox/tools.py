...
from qilin.runtime.secret_context import read_active_secrets
from qilin.runtime.user_context import resolve_runtime_user_id
from qilin.sandbox.exceptions import (
    SandboxError,
    SandboxNotFoundError,
    SandboxRuntimeError,
)
from qilin.sandbox.file_operation_lock import get_file_operation_lock
from qilin.sandbox.overwrite import unwrap_sandbox
...
# Deployments can override via env var QILIN_WRITE_FILE_MAX_BYTES; set to
# 0 (or negative) to disable the guard entirely.
_WRITE_FILE_CONTENT_MAX_BYTES = 80 * 1024
_WRITE_FILE_MAX_BYTES_ENV = "QILIN_WRITE_FILE_MAX_BYTES"
...
def _compiled_mask_patterns(sources: tuple[tuple[str, str], ...]) -> tuple[tuple[re.Pattern[str], str, str], ...]:
    ...
    # ``qilin.sandbox.path_patterns``, which owns that rule so the two copies
...
CHANNEL_USER_ID_ENV = "QILIN_CHANNEL_USER_ID"
...
def _lark_cli_env_from_runtime(runtime: Runtime, command: str, *, sandbox_paths: bool) -> dict[str, str] | None:
    ...
    Settings authorizes ``lark-cli`` under QiLin's per-user integration
...
def _effective_write_file_max_bytes() -> int:
    ...
    Reads ``QILIN_WRITE_FILE_MAX_BYTES`` at call time (not import time)
...
def write_file_tool(
    ...
    Operators can override the cap via env var `QILIN_WRITE_FILE_MAX_BYTES`
