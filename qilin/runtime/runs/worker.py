...
from qilin.runtime.serialization import serialize
from qilin.runtime.stream_bridge import StreamBridge
from qilin.runtime.stream_modes import normalize_stream_modes, to_langgraph_stream_modes
from qilin.runtime.user_context import get_effective_user_id, resolve_runtime_user_id
...
def _install_runtime_context(config: dict, runtime_context: dict[str, Any]) -> None:
    ...
    if isinstance(existing_context, dict):
        ...
        if QILIN_TRACE_METADATA_KEY in runtime_context:
            existing_context.setdefault(QILIN_TRACE_METADATA_KEY, runtime_context[QILIN_TRACE_METADATA_KEY])
...
class _SubagentEventBuffer:
    ...
    async def add(self, chunk: Any) -> None:
        ...
        if self._event_store is None:
            return
        # Lazy import: importing qilin.subagents at module load triggers its
        # package __init__ (executor → agents → tools → task_tool), which imports
        # back from qilin.subagents and deadlocks at gateway startup. Deferring
        # it to call time (after all modules are loaded) breaks that cycle.
        from qilin.subagents.step_events import subagent_run_event
...
async def run_agent(
    ...
    *,
    ...
    # this set to mask out ``qilin_error_fallback`` markers that belong to
    ...
    try:
        ...
        if event_store is not None:
            from qilin.runtime.journal import RunJournal
        ...
        qilin_trace_id = resolve_qilin_trace_id(incoming_metadata.get(QILIN_TRACE_METADATA_KEY))
        if qilin_trace_id:
            runtime_ctx[QILIN_TRACE_METADATA_KEY] = qilin_trace_id
            if is_trace_id_from_request_header():
                merged_metadata = dict(incoming_metadata)
                merged_metadata[QILIN_TRACE_METADATA_KEY] = qilin_trace_id
        ...
        # Shared helper with ``QiLinClient.stream`` so both entry points stay
...
def _try_extract_from_message(obj: Any, pre_existing_ids: set[str] | None = None) -> str | None:
    ...
    if isinstance(additional_kwargs, dict) and additional_kwargs.get("qilin_error_fallback"):
    ...
    if isinstance(obj, dict):
        ...
        if isinstance(nested_kwargs, dict) and nested_kwargs.get("qilin_error_fallback"):
