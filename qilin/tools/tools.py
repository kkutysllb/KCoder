...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use == "qilin.sandbox.tools:bash_tool":
        return True
    return False
...
def get_available_tools(
    ...
    *,
    ...
    `initialize_mcp_tools()` from qilin.mcp module.
    ...
    if getattr(skill_evolution_config, "enabled", False):
        from qilin.tools.skill_manage_tool import skill_manage_tool
    ...
    if include_mcp:
        try:
            ...
            from qilin.mcp.cache import get_cached_mcp_tools
